...
if exist "apps-script\appsscript.json" (dir /b apps-script) else (echo      VACIA - corre UNA-VEZ-1-BAJAR-DE-APPSCRIPT.bat)
echo.
...
echo  [7] URL del Web App
if exist "docs\config.js" (
  findstr /C:"PON_AQUI" docs\config.js >nul 2>&1
  if not errorlevel 1 (echo      FALTA - sigue en placeholder) else (echo      Puesta en docs\config.js)
) else (
  if exist "docs\api.js" (echo      Todavia dentro de docs\api.js - se separa en la fase 2) else (echo      No encontrada - falta mover el frontend a docs)
)
echo.
