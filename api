...
(function () {
  ...
  function GasRunner() { this._success = null; this._failure = null; }

  GasRunner.prototype.withSuccessHandler = function (fn) { this._success = fn; return this._prx(); };
  GasRunner.prototype.withFailureHandler = function (fn) { this._failure = fn; return this._prx(); };

  GasRunner.prototype._exec = function (fnName, args) {
    var self = this;
    fetch(GAS_URL, {
      method: 'POST', headers: { 'Content-Type': 'text/plain' },
      body: JSON.stringify({ fn: fnName, args: args }), redirect: 'follow'
    })
    .then(function (r) { return r.json(); })
    .then(function (d) { if (self._success) self._success(d); })
    .catch(function (e) { if (self._failure) self._failure(e); else console.error('[GAS]', fnName, e); });
  };
