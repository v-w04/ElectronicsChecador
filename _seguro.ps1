...
$archivos = @()
# Los *.js / *.html de la raiz cuentan mientras el frontend no se mueva a docs.
foreach ($p in @('apps-script\*.gs', 'apps-script\*.js', 'docs\*.js', 'docs\*.html', '*.js', '*.html')) {
...
$porForma = @(
    ...
    # La cuenta de servicio de Firebase: ya estuvo pegada en el codigo una vez.
    @{ n = 'llave privada (cuenta de servicio)'; r = 'BEGIN (RSA |EC )?PRIVATE KEY|"private_key"\s*:' }
)
...
# La config WEB de Firebase (apiKey 'AIza...') tambien es publica por diseno:
# es un identificador, no un secreto, y Google la manda al navegador de todos
# modos. Lo que protege es restringirla en Google Cloud (solo el dominio
# github.io) - no esconderla. Solo se perdona en el frontend.
$permitidaFirebaseWeb = 'apiKey\s*:\s*["'']AIza[0-9A-Za-z_\-]{35}["'']'
...
foreach ($a in $archivos) {
    ...
    for ($i = 0; $i -lt $lineas.Count; $i++) {
        ...
        if (-not $saltar -and $a.FullName -notmatch '\\apps-script\\' -and $linea -match $permitidaFirebaseWeb) { $saltar = $true }
        if ($saltar) { continue }
